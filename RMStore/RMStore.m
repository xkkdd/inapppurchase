...
- (int)GetInAppInfoObc:(TProductInfoList*)pplist : (TProductIdList*)pidlist{
 
    __block int ret = 1;
    __block int count = 0;
    __block int i = 0;
    __block TProductInfo* pro;
    
    
  
    NSMutableArray *elements = [NSMutableArray array];
    for (i = 0; i<pidlist->counts; i++) {
       
        [elements addObject:[NSString stringWithUTF8String: pidlist->pidlist[i].pid]];
        printf("in pid[%s]\n",pidlist->pidlist[i].pid);
    }
    NSSet *products = [elements copy];
    
    [[RMStore defaultStore] addStoreObserver:self];
    
    
    [[RMStore defaultStore] requestProducts:products success:^(NSArray *products, NSArray *invalidProductIdentifiers) {
        NSLog(@"Products loaded success[%lu] failed[%lu]",(unsigned long)products.count,(unsigned long)invalidProductIdentifiers.count);
        
        
        count = (int)products.count;
        pro = malloc(sizeof(TProductInfo)*count);
        
        for (i = 0; i<count; i++) {
            SKProduct *product = [[RMStore defaultStore] productForIdentifier:[NSString stringWithUTF8String: pidlist->pidlist[i].pid]];
            
            NSLog(@"load[%@,%@,%@,%@]",product.productIdentifier,product.localizedTitle,product.localizedDescription,[RMStore localizedPriceOfProduct:product]);
            
            strncpy(pro[i].szProductID,[product.productIdentifier UTF8String],125);
            strncpy(pro[i].szDescription,[product.localizedDescription UTF8String],255);
            strncpy(pro[i].szPrice,[[RMStore localizedPriceOfProduct:product] UTF8String],125);
            strncpy(pro[i].szTitle,[product.localizedTitle UTF8String],125);
        ...
        NSLog(@"requestProducts err:%@",error.localizedDescription);
        ret = -1;
    }];
